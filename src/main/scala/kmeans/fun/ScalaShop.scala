...
object ScalaShop {
  ...
  class ScalaShopFrame extends JFrame("ScalaShop\u2122") {
    ...
    stepbutton.addActionListener((_: ActionEvent) => {
      var status = ""
      val time = measure {
        status = canvas.applyIndexedColors(getColorCount, getInitialSelectionStrategy, getConvergenceStrategy)
      }
    ...
    saveMenuItem.addActionListener((_: ActionEvent) => {
    ...
    def getConvergenceStrategy: ConvergenceStrategy =
