...
class IndexedColorFilter(initialImage: Img,
  ...
  val points: Seq[Point] = imageToPoints(initialImage)
  val means: Seq[Point] = initializeIndex(colorCount, points)

  /* The work is done here: */
  private val newMeans = kMeans(points, means, 0.01)

  /* And these are the results exposed */
  def getStatus: String = s"Converged after $steps steps."
  ...
  private def indexedImage(img: Img, means: ParSeq[Point]): Img = {
  ...
  private def indexedImage(img: Img, means: Seq[Point]): Img = {
    val dst = new Img(img.width, img.height)
    val pts = collection.mutable.Set[Point]()
